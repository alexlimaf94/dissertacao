...
#define BUFFER_SIZE                                 52 // Tamanho máx do payload LoRa
...
void OnRxDone( uint8_t *payload, uint16_t size, int16_t rssi_correto, int8_t snr ) {
  ...
  sscanf(rxpacket, "%hd,%f,%f,%f,%f,%hd,%ld,%f,%f", 
         &txNumber, &inclinacaoX, &inclinacaoY, &temp, &humidity, &velocidade_int, &altura, &busvoltage, &current_mA);
  yield();
  ...
  Serial.print("Temperatura Nó-sensor Hidrológico: "); Serial.print(temp); Serial.println(" ºC"); // Unidade de medida: graus Celsius (ºC)
  Serial.print("Umidade Nó-sensor Hididrológico: "); Serial.print(humidity); Serial.println(" %"); // Unidade de medida: porcentagem (%)
  ...
  Serial.print("Temperatura Nó-sensor Meteorológico: "); Serial.print(temperatura); Serial.println(" ºC"); // Unidade de medida: graus Celsius (ºC)
  Serial.print("Umidade Nó-sensor Meteorológico: "); Serial.print(umidade); Serial.println(" %"); // Unidade de medida: porcentagem (%)
...
void setup() {
  ...
  // Abrir o arquivo para escrita
	...
	if (file.size() == 0) {
	  file.println("txNumber,DataHora,InclinacaoX,InclinacaoY,TempNoHid,UmidadeNoHid,VelocidadeEscoamento,NivelAgua,Tensao,Corrente,VelocidadeVento,TempNoMet,UmidadeNoMet,ChuvaInstante,ChuvaAcumulada"); // Escreve o cabeçalho apenas se o arquivo estiver vazio
	}
...
void loop() {
  ...
  if (displayActive && (millis() - displayStartTime >= 1800)) {
    ...
    oleddispay.drawString(0, 20, "Niv: " + String(altura) + "cm,  Vel: " + String(velocidade, 1) + " m/s");
