...
void setup() {
  ...
  Serial.println("Sistema de monitoramento de córrego iniciado...");
...
void loop() {
  ...
  if (sensoresAtivados) {
    ...
    while (millis() - tempoInicio < intervalo) {
      ...
      int altura = 600 - distancia; // Altura da lâmina d'água com o leito do rio 6 m abaixo do sensor.
      ...
      // Exibir no display OLED
      oleddisplay.drawString(0, 40, "Dist: " + String(distancia) + " cm");
      ...
      if (lora_idle == true) {
        ...
        snprintf(txpacket, sizeof(txpacket), "%d,%s,%s,%s,%s,%d,%ld,%s,%s", 
                ...
                distancia, busvoltageStr.c_str(), current_mAStr.c_str());
